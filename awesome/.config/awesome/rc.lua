...
awful.screen.connect_for_each_screen(function(s)
    ...
    -- Create the wibox
    if s.index == 1 then
        --s.mywibox = awful.wibar({ position = "top", screen = s , width = 1120})
        s.mywibox = wibox({ position = "top", screen = s, visible = true, ontop = true, height = 21, width = 1120})
        awful.placement.align(s.mywibox, { position = "top_right", honor_workarea = true })
        s.top_filler = awful.wibar({ position = "top", screen = s, height = 21, opacity = 0, ontop = false})
        s.bottom_filler = awful.wibar({ position = "bottom", screen = s, height = 21, opacity = 0, ontop = false})
    else
...
globalkeys = awful.util.table.join(
    ...
    awful.key({ modkey,           }, "h", function () awful.client.focus.global_bydirection("left") end,
              {description = "focus the next client on the left", group = "client"}),
    awful.key({ modkey,           }, "l", function () awful.client.focus.global_bydirection("right") end,
              {description = "focus the next client on the right", group = "client"}),

    -- Moving clients
    awful.key({ modkey, "Shift"   }, "j", function () awful.client.swap.global_bydirection("down") end,
              {description = "swap with the next client below", group = "client"}),
    awful.key({ modkey, "Shift"   }, "k", function () awful.client.swap.global_bydirection("up") end,
              {description = "swap with the next client above", group = "client"}),
    awful.key({ modkey, "Shift"   }, "h", function () awful.client.swap.global_bydirection("left") end,
              {description = "swap with the next client on the left", group = "client"}),
    awful.key({ modkey, "Shift"   }, "l", function () awful.client.swap.global_bydirection("right") end,
              {description = "swap with the next client on the right", group = "client"}),

    -- Layout manipulation
    awful.key({ modkey,           }, "u", function () awful.screen.focus_bydirection("left") end,
